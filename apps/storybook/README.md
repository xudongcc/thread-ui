...
Use the language toolbar to check English and Chinese in the same story; do not add separate language-specific stories. Calendar's Default and AppProvider's Feedback interaction checks follow the selected language. Language selection changes component translations; it does not translate arbitrary story args. Theme selection applies to the preview document, including portalled popups. The shared theme lives in `themes/default-theme/default.css`; application-specific CSS stays in each app. Storybook uses system fonts; the docs app continues to load its web fonts.
