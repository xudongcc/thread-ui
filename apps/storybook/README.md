...
Use the language toolbar to check English and Chinese in the same story; do not add separate language-specific stories. Calendar's Default and AppProvider's Feedback interaction checks follow the selected language. Language selection changes component translations; it does not translate arbitrary story args. Theme selection applies to the preview document, including portalled popups. The shared theme lives in `packages/styles/theme.css`; application-specific CSS stays in each app. Storybook uses system fonts; the docs app continues to load its web fonts.
...
Check the Code panel as well as the canvas. A helper used as JSX (`render: args => <Example {...args} />`) exposes only that helper name. For a shared render function, use `render: Example` so dynamic source shows the public components it returns. Function APIs and complex workflows should live in `stories/examples/*.tsx`; import the same file with `?raw` into `parameters.docs.source.code` to show the actual implementation, including hooks and handlers, without maintaining a separate snippet. Override that source for stories with a different implementation. Simple Props API stories should keep dynamic source so Controls update the displayed props.
