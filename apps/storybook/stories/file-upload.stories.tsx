...
export const AutomaticUpload: Story = {
  parameters: { docs: { source: { code: workflowSource, language: "tsx" } } },
  args: { onUpload: simulateUpload },
  render: UploadExample,
};
export const ManualUpload: Story = {
  parameters: {
    docs: {
      source: { code: workflowSource, language: "tsx" },
      description: {
        story:
  ...
  render: UploadExample,
  args: { onUpload: simulateUpload, autoUpload: false, concurrency: 1 },
  play: async ({ canvas, canvasElement, userEvent }) => {
    await userEvent.upload(
