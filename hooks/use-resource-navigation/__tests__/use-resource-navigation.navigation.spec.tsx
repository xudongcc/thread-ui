...
const conditions = {
  query: "example*",
  filter: { prefix: { $eq: "sk" } },
  orderBy: { field: ResourceOrderField.ID, direction: OrderDirection.ASC },
};
function neighbors(
  previous?: string,
  next?: string,
...
function saveSearch(
  search: z.input<typeof resourceSearchSchema>,
  key = resourceKey,
) {
  const saved = renderHook(() =>
    useResourceNavigation({ key, searchSchema: resourceSearchSchema }),
  );
...
describe("useResourceNavigation", () => {
  it("reads defaults without querying or recording a list visit when query is omitted", async () => {
    ...
    expect(result.current.search).toEqual(resourceSearchSchema.parse({}));
  ...
  it("queries supplied search immediately and retains schema-defined view metadata", async () => {
    const searchSchema = resourceSearchSchema.and(
  ...
  it("executes the lazy-query closure for changed conditions or closures, but not saved pagination", async () => {
    ...
    expect(query).toHaveBeenCalledTimes(1);
    expect(query).toHaveBeenLastCalledWith({
      search: resourceSearchSchema.parse({}),
    });
    ...
    type Options = Parameters<
      typeof useResourceNavigation<typeof resourceSearchSchema>
    >[0];
    ...
    expectTypeOf<Parameters<NonNullable<Options["query"]>>[0]>().toEqualTypeOf<{
      search: z.output<typeof resourceSearchSchema>;
    }>();
  ...
  it("retains saved search during loading and failure, and retries without page-owned query state", async () => {
    const search = resourceSearchSchema.parse({
      ...conditions,
      first: 5,
      after: "old",
    });
  ...
  it.each([
    ...
    async (pagination) => {
      ...
      await waitFor(() => expect(result.current.loading).toBe(false));
      expect(query).toHaveBeenLastCalledWith({
        search: resourceSearchSchema.parse({ ...conditions, ...pagination }),
      });
      expect(result.current.backSearch).toEqual({
        ...conditions,
        first: 5,
  ...
  it("uses the current record inside the query closure across record changes and browser back", async () => {
    saveSearch({
      orderBy: {
        field: ResourceOrderField.LAST_USED_AT,
  ...
  it("uses schema defaults on direct entry even after neighbors load, without writing a fake list visit", async () => {
    ...
    expect(result.current.backSearch).toEqual({
      first: 20,
      orderBy: {
        field: ResourceOrderField.CREATED_AT,
        direction: OrderDirection.DESC,
      },
    });
    expect(query).toHaveBeenLastCalledWith({
